...
export default function WhatsAppWidget() {
    ...
    // Secara default OPEN untuk desktop (Persistent)
    ...
    // Tombol pemicu jika sidebar ditutup manual
    if (!isOpen) {
        return (
            <button 
                onClick={() => setIsOpen(true)}
                className="fixed bottom-6 right-6 z-[9999] flex items-center justify-center w-14 h-14 bg-[#25D366] text-white rounded-full shadow-2xl hover:scale-110 transition-all"
            >
    ...
    return (
        /* Gunakan sticky h-screen agar sidebar mengikuti scroll halaman namun tetap di posisinya.
        ...
        */
        <div className="fixed lg:relative top-0 right-0 h-screen w-full lg:w-[450px] shrink-0 bg-background border-l border-border z-[50] flex flex-col overflow-hidden shadow-xl lg:shadow-none transition-all duration-300">
            ...
            {/* Container Body dengan Independent Scroll Area */}
            <div className="flex-1 flex flex-col overflow-hidden relative">
                ...
                {viewMode === 'chat' && (
                    <div className="flex-1 flex flex-col overflow-hidden relative">
                                        ...
                                        <div className="overflow-hidden rounded-xl">
                                            ...
                                            {msg.message_type === 'video' && msg.media_url && (
                                                <video 
                                                    src={msg.media_url.startsWith('http') ? msg.media_url : `${baseGowaUrl}/${msg.media_url}`} 
                                                    controls 
                                                    className="max-w-full rounded-lg max-h-64 bg-black" 
                                                />
                                            )}
                                            ...
                                            {msg.message_type === 'document' && msg.media_url && (
                                                <a 
                                                    ...
                                                    target="_blank" 
                                                    rel="noreferrer" 
                                                    className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg m-1 border border-dashed border-slate-300 hover:bg-slate-100 transition"
                                                >
                                                    ...
                                                    <div className="flex flex-col min-w-0">
                                                        <span className="text-[11px] font-bold truncate text-slate-700">{msg.file_name || 'Download PDF'}</span>
                                                        <span className="text-[9px] text-slate-400 uppercase">Lihat Dokumen</span>
                                                    </div>
                                                </a>
                                            )}
                                        </div>
                        ...
                        {/* Input Area - Tetap di Bawah Chat Area */}
