...
export default function WhatsAppWidget() {
    ...
    // Desktop default TRUE, Mobile default FALSE
    ...
    // Effect untuk Persistent Sidebar Desktop
    useEffect(() => {
        const handleResize = () => {
        ...
        return (
            <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end gap-2 lg:hidden">
                <button 
                    ...
                    className="group relative flex items-center justify-center w-14 h-14 bg-[#25D366] text-white rounded-full shadow-2xl hover:scale-110 transition-all"
                >
                    {totalUnread > 0 && <span className="absolute -top-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full bg-red-600 text-[11px] font-bold text-white border-2 border-white">{totalUnread > 99 ? '99+' : totalUnread}</span>}
    ...
    return (
        /* Container utama diatur h-full agar mengikuti parent (h-screen di Layout). 
            lg:relative menjadikannya sidebar tetap di desktop.
        */
        <div className="fixed lg:relative top-0 right-0 h-full w-full lg:w-[450px] shrink-0 bg-background border-l border-border z-[50] flex flex-col overflow-hidden shadow-xl lg:shadow-none transition-all duration-300">
            ...
            <div className="h-16 bg-background text-foreground border-b border-border p-4 flex justify-between items-center shadow-sm shrink-0 z-20">
                ...
                <div className="flex gap-3 items-center shrink-0">
                    ...
                    <button onClick={() => setIsOpen(false)} className="hover:bg-accent hover:text-accent-foreground p-1 rounded-md transition-colors lg:hidden">
